...
class TemplateSettings(models.Model):
    ...
    template_inv = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'customized_invoice.INVOICE\_%\_document' )]",
        required=False)
    template_so = fields.Many2one(
        ...
        domain=
        "[('type', '=', 'qweb'), ('key', 'like', 'customized_invoice.SO\_%\_document' )]",
        required=False)
    ...
    logo = fields.Binary(
        ...
        attachment=True,
        help=
        "This field holds the image used as logo for the reports, if non is uploaded, the company logo will be used"
    )
    footer_logo = fields.Binary(
        "Footer Logo",
        attachment=True,
        help=
        "This field holds the image used as footer logo for the reports, if non is uploaded and footer logo is enabled then the company logo will be used"
    )
    odd = fields.Char(
        'Odd parity Color',
        size=7,
    ...
    theme_txt_color = fields.Char(
        ...
        default="#FFFFFF",
        help=
        "The Text color of the areas bearing the theme color. Normally this should NOT be the same color as the\
    ...
    aiw_report = fields.Boolean(
        ...
        default=True,
        help=
        "Check this box to enable the display of amount in words in the invoice/quote/sale order reports"
    )
    show_img = fields.Boolean(
        'Enable product Image',
        default=True,
        help=
        "Check this box to display product image in Sales Order, Quotation, Invoice and Delivery Note"
    )
    ...
    footer = fields.Boolean(
        ...
        default=True,
        help=
        "Check this box to enable footer in your reports. You may want to disable footer if you are using a watermark PDF with a footer content already"
    )
